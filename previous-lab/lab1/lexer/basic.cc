...
static inline void check_gc_init()
{
...
string::string( const string& b ) { sp = b.sp; };

string::string( const char* s ) 
{
    check_gc_init();
    sp = new (GC) char[strlen(s)+1]; strcpy(sp,s); 
};
...
ostream& operator << ( ostream& s, const string& a ) { return s << a.content(); };
...
{
    static char formBuffer[64];

    va_list argptr;
    va_start(argptr, fmt);
    {
        vsprintf(formBuffer, fmt, argptr);
    }
    va_end(argptr);
