...
extern int lineno;
extern int columnno;
extern int parse_error; // defined in pcat.y
extern int show_ast;
...
%%
                     ...
                     start: program { if(show_ast) { print_ast($1); printf("\n"); } ast_root = $1; };
                   ...
                   program: PROGRAM IS body SEMICOLON { $$ = $3 };
                      ...
                      body: declaration_array BEGINT statement_array END        { $$ = mk_node(&(@$), body_node, clst(2, mk_node(&(@1), genr_node, reverse($1)), mk_statblock(&(@3), $3))); }
                          | declaration_array error BEGINT statement_array END  { $$ = NULL; parse_error = 1; };
         ...
         declaration_array: /* empty */                                     { $$ = NULL; }
                  ...
                  var_decl: id id_array_optext opt_typename ASSIGN expression SEMICOLON     { $$ = mk_node(&(@$), var_decl, clst(3, mk_idlist(&(@1), &(@2), $1, $2), $3, $5)); }
                          | id id_array_optext opt_typename ASSIGN error SEMICOLON          { $$ = NULL; parse_error = 1 };

            var_decl_array: /* empty */                                                     { $$ = NULL; }
                          | var_decl_array var_decl                                         { $$ = cons($2, $1); };

                 type_decl: type_name IS compound_type SEMICOLON    { $$ = mk_node(&(@$), type_decl, clst(2, $1, $3)); }
                          | type_name IS error SEMICOLON            { $$ = NULL; parse_error = 1 };
           type_decl_array: /* empty */                             { $$ = NULL; }
             ...
             compound_type: ARRAY OF type_name                      { $$ = mk_node(&(@$), arr_type, clst(1, $3)); }
                          ...
                          | RECORD error END                        { $$ = NULL; parse_error = 1 };
      ...
      formal_params_optext: /* empty */                                         { $$ = NULL; }
                          ...
                          | formal_params_optext error SEMICOLON formal_section { $$ = NULL; parse_error = 1; }

            formal_section: id id_array_optext COLON type_name                  { $$ = mk_node(&(@$), param_sec, clst(2, mk_idlist(&(@1), &(@2), $1, $2), $4)); }
                          | error COLON type_name                               { $$ = NULL; parse_error = 1 };
                 ...
                 statement: l_value ASSIGN expression SEMICOLON                 { $$ = mk_node(&(@$), asgn_stat, clst(2, $1, $3)); }
                          ...
                          | IF expression THEN 
                            ...
                            END SEMICOLON                                       { 
                                                                                    $$ = mk_node(&(@$), if_stat, 
                                                                                        clst
                                                                                        (
                                                                                            3, mk_noder(&(@1), &(@4), if_node, clst(2, $2, mk_statblock(&(@4), $4))),
                                                                                            mk_node(&(@5), elseifs_node, reverse($5)),
                                                                                            $6
                                                                                        )
                                                                                    ); 
                                                                                }
                          ...
                          | error SEMICOLON                                     { $$ = NULL; parse_error = 1;  /*yyerrok; <- do not suspend next few errors */ };

           statement_array: /* empty */                 { $$ = NULL; }
                          | statement_array statement   { $$ = cons($2, $1); };

   statement_if_opt_elseif: /* empty */                                                     { $$ = NULL; }
                          | statement_if_opt_elseif ELSIF expression THEN statement_array   { $$ = cons(mk_noder(&(@2), &(@5), if_node, clst(2, $3, mk_statblock(&(@5), $5))), $1); }
                          | statement_if_opt_elseif ELSIF error THEN statement_array        { $$ = NULL; parse_error = 1 };

     statement_if_opt_else: /* empty */             { $$ = NULL; }
                          | ELSE statement_array    { $$ = mk_statblock(&(@$), $2); };
 ...
 statement_return_opt_expr: /* empty */ { $$ = NULL; }
          ...
          read_args_optext: /* empty */                                 { $$ = NULL; }
                          ...
                          | read_args_optext error COMMA l_value        { $$ = NULL; parse_error = 1 };
         ...
         write_args_optext: /* empty */                                 { $$ = NULL; }
                          ...
                          | write_args_optext error COMMA write_expr    { $$ = NULL; parse_error = 1 };
                ...
                expression: number                          { $$ = $1; }
                   ...
                   l_value: id
                          ...
                          | l_value LBRACKET error RBRACKET         { $$ = NULL; parse_error = 1 }
          ...
          arguments_optext: /* empty */                                 { $$ = NULL; }
                          ...
                          | arguments_optext error COMMA expression     { $$ = NULL; parse_error = 1 };
        ...
        field_inits_optext: /* empty */                                             { $$ = NULL; }
                          ...
                          | field_inits_optext error COMMA id ASSIGN expression     { $$ = NULL; parse_error = 1 };
       ...
       array_values_optext: /* empty */                                         { $$ = NULL; }
                          ...
                          | array_values_optext error COMMA array_value         { $$ = NULL; parse_error = 1 };
           ...
           id_array_optext: /* empty */                     { $$ = NULL; }
                          ...
                          | id_array_optext error COMMA id  { $$ = NULL; parse_error = 1 };
